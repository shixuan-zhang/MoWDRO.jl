...
NUM_PART    = Int(PROB_CFG["number of parts"])
NUM_PROD    = Int(PROB_CFG["number of products"])
PRICE_MIN   = Float64(PROB_CFG["minimum price"])
PRICE_MAX   = Float64(PROB_CFG["maximum price"])
COST_MIN    = Float64(PROB_CFG["minimum cost"])
COST_MAX    = Float64(PROB_CFG["maximum cost"])
LATE_RATIO  = Float64(PROB_CFG["late ratio"])
SALVAGE_MAX = Float64(PROB_CFG["maximum salvage"])
DEMAND_MIN  = Float64(PROB_CFG["minimum demand"])
DEMAND_MAX  = Float64(PROB_CFG["maximum demand"])
DEMAND_VAR  = Float64(PROB_CFG["demand variance"])
STORAGE_MAX = Float64(PROB_CFG["maximum storage"])
...
function experiment_production(
    ...
    # take the samples of salvage prices and demands; pre-draw one full
    # N_max-sized training set per replication, so each rep gets independent
    # training data while all per-rep slices of size N share the same prefix.
    N_max = maximum(train_sizes)
    sample_train_full_per_rep = [
        [round.([exp.(randn(m).*σ);rand(n)],digits=NUM_DIG) for _ in 1:N_max]
        for _ in 1:num_reps
    ]

...
Run the default MoWDRO-only configuration with 4 workers:
...
| --- | --- | --- |
| `production.toml` | `"none"` (MoWDRO only) | 20 parts, 10 products, `"radius scaling" = 2`, 3 replications. |
...
`(N, r)` combination is solved by MoWDRO alongside both the copositive-SDP
baseline and the nonconvex-QCQP baseline.

## Output columns

The CSVs written by these experiment scripts always contain the MoWDRO columns

...
const FEAS_TOL = 1.0e-1
const NUM_TRAIN = 10 
const NUM_TEST = 10000 
const WASS_INFO = [WassInfo(0.0,DEG_POLY*2),
                   WassInfo(1e-4,DEG_POLY*2),
                   WassInfo(1e-3,DEG_POLY*2),
                   WassInfo(1e-2,DEG_POLY*2)]

OUTPUT_FILE = "../result_regression_$(NUM_VAR)_$(DEG_POLY).csv"
if length(ARGS) > 0
    OUTPUT_FILE = ARGS[1]
end
...
function experiment_regression(
        ...
        support_set::String = "orthant",    # string for the support set Z, options are
                                            # (1) "fullspace" ℝᵐ, (2) "orthant" [0,∞)ᵐ, (3) "box" [0,1]ᵐ
        sparse_prob::Float64 = 0.0,         # probability of setting coefficients to zero for sparsity
    ...
    sample_train = map(augment_sample, [cholesky(Σ).L * randn(m) for _ in 1:N])
    sample_test = map(augment_sample, [cholesky(Σ).L * randn(m) for _ in 1:N])
...
experiment_regression(NUM_VAR, DEG_POLY, 0.1, C'*C, WASS_INFO, sparse_prob=0.5)
